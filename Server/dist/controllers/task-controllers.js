...
const patchAbortTask = async (req, res, next) => {
    //remove this if you dont have validation
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return next(new HttpError("Invalid inputs", 422));
...
const patchCompleteTask = async (req, res, next) => {
    //remove this if you dont have validation
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
        return next(new HttpError("Invalid inputs", 422));
...
export { getTasksByProject, postFetchCurrentTask, postAddFirstTask, postAddDirectTask, patchAbortTask, patchCompleteTask, patchUpdateTask, };
